...
;; Due to buggy implementation of c-define we need to force the imports via a dummy function
#;(define (%force-imports%)
  (setup-hook)
  (main-loop-hook))

(c-define (c-setup) () int "setup_scheme_hook" "" (anygame_1#setup-hook))
(c-define (c-draw) () int "draw_scheme_hook" "" (anygame_1#main-loop-hook))
...
// <-- BEGIN GAMBIT CONFIGURATION

#define LINKER ____20_main_2d_android
...
static int engine_init_display(struct engine* engine) {
    ...
    surface = eglCreateWindowSurface(display, config, (NativeWindowType)engine->app->window, NULL);
...
(c-define-type android-engine (struct "engine"))
(c-define-type android-engine* (pointer "engine"))


(define android:initialize
  (c-lambda (android-engine*) int
#<<end-of-c-lambda
___result = engine_init_display (___arg1);
end-of-c-lambda
))
